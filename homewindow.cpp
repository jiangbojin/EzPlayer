...
HomeWindow::HomeWindow(QWidget *parent)
  ...
  // 初始化双渲染器
  display_gl_ = ui->display; // UI 中已经创建的 OpenGL 控件
  display_sw_ = new DisplayWind(ui->centralWidget);
  display_sw_->setObjectName("display_sw");
  display_sw_->setGeometry(display_gl_->geometry());
  display_sw_->setMinimumSize(display_gl_->minimumSize());
  display_sw_->hide(); // 默认隐藏软件渲染器
  use_opengl_render_ = true;

  // 创建渲染模式切换按钮
  QPushButton *renderSwitchBtn = new QPushButton("渲染:OpenGL", ui->ctrlBar);
  renderSwitchBtn->setObjectName("renderSwitchBtn");
  renderSwitchBtn->setMinimumSize(QSize(90, 30));
  renderSwitchBtn->setMaximumSize(QSize(90, 30));
  ui->gridLayout_2->addWidget(renderSwitchBtn, 0, 15, 1, 1);
  connect(renderSwitchBtn, &QPushButton::clicked, this,
          &HomeWindow::on_renderSwitchBtn_clicked);
}
...
int HomeWindow::InitSignalsAndSlots() {
  ...
  // 播放文件
  connect(ui->playList, &Playlist::SigPlay, this,
          &HomeWindow::play); // 绑定列表选的的文件播放
  // 被动停止
  ...
  // 进度条位置调整
  connect(this, &HomeWindow::sig_updateCurrentPosition, this,
          &HomeWindow::on_updateCurrentPosition);
  ...
  // 设置更新音量条的回调
  connect(ui->playSlider, &CustomSlider::SigCustomSliderValueChanged, this,
          &HomeWindow::on_playSliderValueChanged); // 进度条功能seek
  connect(ui->volumeSlider, &CustomSlider::SigCustomSliderValueChanged, this,
          &HomeWindow::on_volumeSliderValueChanged); // 音量调整
  ...
  // ui显示av cache
  connect(this, &HomeWindow::sig_updateAudioCacheDuration, this,
          &HomeWindow::on_UpdateAudioCacheDuration);
  connect(this, &HomeWindow::sig_updateVideoCacheDuration, this,
          &HomeWindow::on_UpdateVideoCacheDuration);

  // 打开文件
  connect(ui->openFileAction, &QAction::triggered, this,
          &HomeWindow::on_openFile);
  connect(ui->openUrlAction, &QAction::triggered, this,
          &HomeWindow::on_openNetworkUrl);

  // 播放和暂停
  connect(this, &HomeWindow::sig_updatePlayOrPause, this,
          &HomeWindow::on_updatePlayOrPause);

  // 缓存阀门
  QObject::connect(ui->bufDurationBox, &QComboBox::currentTextChanged, this,
                   &HomeWindow::on_updateDurationCacheMax);
  QObject::connect(ui->jitterBufBox, &QComboBox::currentTextChanged, this,
                   &HomeWindow::on_updateDurationCacheMin);

  // 硬件编码
  QObject::connect(ui->cbx_hwdecode, &QComboBox::currentTextChanged, this,
                   &HomeWindow::on_updateHW_DecodeType);
  // 最大尝试连接时间
  QObject::connect(ui->cbx_network_tm, &QComboBox::currentTextChanged, this,
                   &HomeWindow::on_updateNetWork_TIMEOUT);
  return 0;
}
/**
 *  事件循环回调，从ijk取出消息做处理
...
int HomeWindow::OutputVideo(const Frame *f) {
  ...
  if (use_opengl_render_) {
    return display_gl_->Draw(f);
  } else {
    return display_sw_->Draw(f);
  }
}

void HomeWindow::resizeEvent(QResizeEvent *event) { resizeUI(); }
...
void HomeWindow::resizeUI() {
  ...
  LOG(INFO) << "listBtn: " << ui->listBtn->geometry().x() << " "
            << ui->listBtn->geometry().y();
  ...
  LOG(INFO) << "settingBtn: " << ui->settingBtn->geometry().x() << " "
            << ui->settingBtn->geometry().y();
  // 设置显示画面
  ...
  int y1 = 0;
  activeDisplay()->setGeometry(0, y1, width, height);
  // 设置文件列表 list
  if (is_show_file_list_) {
    ui->playList->setGeometry(activeDisplay()->width(), y1,
                              ui->playList->width(), height);
  }
...
void HomeWindow::on_openFile() {
  QUrl url = QFileDialog::getOpenFileName(
      this, QStringLiteral("选择路径"), QDir::homePath(), nullptr, nullptr,
...
void HomeWindow::reqUpdateCacheDuration() {
  ...
  if (cur_time - pre_get_cache_time_ > 500) {
    ...
    if (mp_) {
      // 更改ui
      audio_cache_duration = mp_->ijkmp_get_property_int64(
          FFP_PROP_INT64_AUDIO_CACHED_DURATION, 0);
      video_cache_duration = mp_->ijkmp_get_property_int64(
          FFP_PROP_INT64_VIDEO_CACHED_DURATION, 0);
...
bool HomeWindow::play(std::string url) {
  ...
  // 视频刷新回调 display
  mp_->AddVideoRefreshCallback(
      std::bind(&HomeWindow::OutputVideo, this, std::placeholders::_1));
  // 1.2 设置url
  ...
  if (use_opengl_render_) {
    display_gl_->StartPlay();
  } else {
    display_sw_->StartPlay();
  }
  this->Start(); // 开启事件循环
  startTimer();  // 每秒cb
...
void HomeWindow::on_updateCurrentPosition(long position) {
  ...
  if (total_duration_ > 0) {
    int pos =
        current_position_ * 1.0 / total_duration_ * ui->playSlider->maximum();
...
void HomeWindow::on_playSliderValueChanged(int value) { seek(value); }
...
bool HomeWindow::stop() {
  ...
  if (use_opengl_render_) {
    display_gl_->StopPlay();
  } else {
    display_sw_->StopPlay();
  }
  ui->playOrPauseBtn->setIcon(QIcon(":/res/pause.png"));
  ui->playOrPauseBtn->setIconSize(QSize(30, 30));
...
void HomeWindow::on_forwardFastBtn_clicked() { fastForward(MP_SEEK_STEP); }

void HomeWindow::on_backFastBtn_clicked() { fastBack(-1 * MP_SEEK_STEP); }
...
QWidget *HomeWindow::activeDisplay() {
  if (use_opengl_render_) {
    return static_cast<QWidget *>(display_gl_);
  } else {
    return static_cast<QWidget *>(display_sw_);
  }
}

void HomeWindow::on_renderSwitchBtn_clicked() {
  // 播放中不允许切换
  if (mp_) {
    emit sig_showTips(Toast::WARN, "请先停止播放再切换渲染模式");
    return;
  }

  use_opengl_render_ = !use_opengl_render_;

  QPushButton *btn = findChild<QPushButton *>("renderSwitchBtn");

  if (use_opengl_render_) {
    // 切换到 OpenGL
    display_sw_->hide();
    display_gl_->setGeometry(display_sw_->geometry());
    display_gl_->show();
    if (btn)
      btn->setText("渲染:OpenGL");
  } else {
    // 切换到软件渲染
    display_gl_->hide();
    display_sw_->setGeometry(display_gl_->geometry());
