...
//            LogInfo("RTMPPacket_m_packetType:%d\n", packet.m_packetType);
    ...
    //                            packet.m_nBodySize);
                ...
                {
                    LOG(INFO)<<"%s:%s:k:%d:t:%u"
                            << play_time->getRtmpTag()
...
//                            memcpy(&nalu_pkt.data[0], nalu_header_4bytes, 4);
                            if(keyframe) {
                                // 先拷贝 sps
                                memcpy(nalu_pkt.data, (uint8_t *)sps_vector_[0].c_str(), sps_vector_[0].size());
                                // pps
                                memcpy(nalu_pkt.data + sps_vector_[0].size(), (uint8_t *)pps_vector_[0].c_str(), pps_vector_[0].size());
                                // i帧
                                memcpy(nalu_pkt.data+ sps_vector_[0].size()+ pps_vector_[0].size(),
