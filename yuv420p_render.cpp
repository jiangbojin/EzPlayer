...
void YUV420P_Render::initialize()
{
    ...
    //定义顶点着色器和片元着色器的源代码字符串
    //顶点shader  将顶点位置和纹理坐标传递给片元着色器。
    const char *vString =
    ...
    //片元shader 将 YUV 颜色空间转换为 RGB 颜色空间,并输出最终的像素颜色。
    const char *tString =
            "varying vec2 texture_Out;\
    ...
    // m_program加载shader（顶点和片元）脚本 加载并编译着色器程序
    //片元（像素） 加载顶点着色器和片元着色器
    ...
    //从shader获取地址 获取着色器中 Y、U、V 纹理采样器的位置
