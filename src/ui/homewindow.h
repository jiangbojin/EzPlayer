...
class HomeWindow : public QMainWindow, public CommonLooper {
   ...
   private:
    ...
    // OPENGL测试
    bool use_opengl_render_          = true;     // 默认使用 OpenGL 渲染
    DisplayWind* display_sw_         = nullptr;  // 软件渲染控件
    OpenGLDisplayWidget* display_gl_ = nullptr;  // OpenGL 渲染控件
   ...
   */
   ...
   private slots:
    ...
    // 渲染模式切换
    void on_renderSwitchBtn_clicked();
   ...
   private:
    ...
    /// @brief 获取当前活动的显示控件指针（作为 QWidget*）
    QWidget* activeDisplay();
    // 主动获取信息，并更新到ui
