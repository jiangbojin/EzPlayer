...
HomeWindow::HomeWindow(QWidget* parent) : QMainWindow(parent), ui(new Ui::HomeWindow) {
    ...
    connect(renderSwitchBtn, &QPushButton::clicked, this, &HomeWindow::on_renderSwitchBtn_clicked);
}
...
void HomeWindow::resizeUI() {
    ...
    // 设置文件列表 list
    if (is_show_file_list_) {
        ui->playList->setGeometry(activeDisplay()->width(), y1, ui->playList->width(), height);
    }
...
void HomeWindow::on_openFile() {
    QUrl url =
        QFileDialog::getOpenFileName(this, QStringLiteral("选择路径"), QDir::homePath(), nullptr,
    ...
    if (cur_time - pre_get_cache_time_ > 500) {
        ...
        if (mp_) {
            ...
            audio_cache_duration =
                mp_->ijkmp_get_property_int64(FFP_PROP_INT64_AUDIO_CACHED_DURATION, 0);
            video_cache_duration =
                mp_->ijkmp_get_property_int64(FFP_PROP_INT64_VIDEO_CACHED_DURATION, 0);
            ...
            if (hw_decode != "未选择") {
                LOG(INFO) << "hw_decoder_gpu_tocpu_copy SUSS"
                          << "flag" << mp_->Get_ffplayer()->m_isHw_device << " hw_device_type  "
                          << mp_->Get_ffplayer()->hw_device_type << " hw_pix_fmt "
                          << mp_->Get_ffplayer()->hw_pix_fmt;
...
bool HomeWindow::play(std::string url) {
    ...
    // 视频刷新回调 display
    mp_->AddVideoRefreshCallback(std::bind(&HomeWindow::OutputVideo, this, std::placeholders::_1));
    // 1.2 设置url
