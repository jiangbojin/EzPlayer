...
static void freeStreamBuffers(sonicStream stream) {
    if (stream && stream->inputBuffer != NULL) {
...
static int allocateStreamBuffers(sonicStream stream, int sampleRate,
                                 int numChannels) {  // 最小的pitch周期 44100/400 = 110
 ...
 * @param         numChannels   通道数 只支持1和2通道
 * @return        sonicStream
 * @attention
 */
...
static int enlargeOutputBufferIfNeeded(sonicStream stream, int numSamples) {
    if (stream->numOutputSamples + numSamples > stream->outputBufferSize) {
        ...
        stream->outputBuffer = (short*)realloc(
            stream->outputBuffer, stream->outputBufferSize * sizeof(short) * stream->numChannels);
        if (stream->outputBuffer == NULL) {
            return 0;
        }
    }
...
static int enlargeInputBufferIfNeeded(sonicStream stream, int numSamples) {
    ...
    if (stream->numInputSamples + numSamples > stream->inputBufferSize) {
        ...
        stream->inputBuffer = (short*)realloc(
            stream->inputBuffer, stream->inputBufferSize * sizeof(short) * stream->numChannels);
        if (stream->inputBuffer == NULL) {
            return 0;
        }
    }
...
static int copyInputToOutput(sonicStream stream, int position) {
    ...
    if (!copyToOutput(stream, stream->inputBuffer + position * stream->numChannels, numSamples)) {
        return 0;
    }
    // 剩余需要拷贝的输入缓冲区的采样点数
...
/* Overlap two sound segments, ramp the volume of one down, while ramping the
   other one from zero up, and add them, storing the result at the output. */
static void overlapAddWithSeparation(int numSamples, int numChannels, int separation, short* out,
...
static int moveNewSamplesToPitchBuffer(sonicStream stream, int originalNumOutputSamples) {
    ...
    if (stream->numPitchSamples + numSamples > stream->pitchBufferSize) {
        ...
        stream->pitchBuffer = (short*)realloc(
            stream->pitchBuffer, stream->pitchBufferSize * sizeof(short) * numChannels);
        if (stream->pitchBuffer == NULL) {
            return 0;
        }
    }
...
int sonicChangeShortSpeed(short* samples, int numSamples, float speed, float pitch, float rate,
                          float volume, int useChordPitch, int sampleRate,
                          int numChannels) {  // 创建并初始化流
    sonicStream stream = sonicCreateStream(sampleRate, numChannels);
    // 设置流的速度
    sonicSetSpeed(stream, speed);
    // 设置流的音调
