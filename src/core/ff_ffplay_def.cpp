...
static int packet_queue_put_private(PacketQueue* q, AVPacket* pkt) {
    ...
    pkt1->next = NULL;
    if (pkt ==
        &flush_pkt)  //如果放入的是flush_pkt，需要增加队列的播放序列号，以区分不连续的两段数据
    {
        q->serial++;
...
int packet_queue_put(PacketQueue* q, AVPacket* pkt) {
    ...
    ez_mutex_lock(q->mutex);
    ret = packet_queue_put_private(q, pkt);  //主要实现
    ez_mutex_unlock(q->mutex);
...
void packet_queue_abort(PacketQueue* q) {
    ...
    ez_cond_signal(q->cond);  //释放一个条件信号
...
double packet_queue_cache_duration(PacketQueue* q, AVRational time_base, double packet_duration) {
    ...
    packets_duration =
        packet_duration * q->nb_packets;  // packet可能存在多帧音频的情况, 此时这里计算就存在误差
    ...
    for (;;) {
        ...
        if (pkt1) {                     //队列中有数据
            ...
            *pkt = pkt1->pkt;
            ...
            ez_cond_wait(q->cond, q->mutex);
        }
    }
    ez_mutex_unlock(q->mutex);  // 释放锁
    return ret;
}
...
void frame_queue_signal(FrameQueue* f) {
    ez_mutex_lock(f->mutex);
    ez_cond_signal(f->cond);
    ez_mutex_unlock(f->mutex);
}
...
void frame_queue_push(FrameQueue* f) {
    ...
    ez_mutex_lock(f->mutex);
    f->size++;
    ez_cond_signal(f->cond);  // 当_readable在等待时则可以唤醒
    ez_mutex_unlock(f->mutex);
}
