...
static inline void audio_mix_pcm(uint8_t* dst, const uint8_t* src, int len, int volume) {
    if (volume >= 128) {
        memcpy(dst, src, len);
        return;
    }
    ...
    for (int i = 0; i < count; ++i) {
        int32_t val = (static_cast<int32_t>(s[i]) * volume) / 128;
        d[i]        = static_cast<int16_t>(std::clamp(val, -32768, 32767));
    }
}
...
FFPlayer::FFPlayer(std::shared_ptr<MessageQueue> m) : msg_queue_(m) {
...
int FFPlayer::stream_open(const char* file_name) {
    ...
    startup_volume = av_clip(AUDIO_MIX_MAXVOLUME * startup_volume / 100, 0, AUDIO_MIX_MAXVOLUME);
    ...
    // 创建视频刷新线程video_refresh_thread
    video_refresh_thread_ = std::make_unique<std::thread>(&FFPlayer::video_refresh_thread, this);
...
int FFPlayer::stream_component_open(int stream_index) {
    ...
    switch (avctx->codec_type) {
        ...
        case AVMEDIA_TYPE_VIDEO:
            ...
            // 解码器信息
            decode_type = avcodec_get_name(avctx->codec_id);
...
static int audio_decode_frame(FFPlayer* is) {
    ...
    data_size = av_samples_get_buffer_size(NULL, frame_channels, af->frame->nb_samples,
    ...
    if (af->frame->format != is->audio_src.fmt ||                                 // 采样格式
        ...
        ret = swr_alloc_set_opts2(&is->swr_ctx,
                                  ...
                                  &dec_layout,  // 数据源
                                  (enum AVSampleFormat)af->frame->format, af->frame->sample_rate, 0,
                                  NULL);
        if (ret < 0 || !is->swr_ctx || (ret = swr_init(is->swr_ctx)) < 0) {
            ...
            snprintf(errstr, sizeof(errstr),
                     ...
                     av_get_sample_fmt_name((enum AVSampleFormat)af->frame->format), frame_channels,
    ...
    if (is->swr_ctx) {
        ...
        if (len2 ==
            out_count) {  // 这里的意思是我已经多分配了buffer，实际输出的样本数不应该超过我多分配的数量
...
static void sdl_audio_callback(void* opaque, uint8_t* stream, int len) {
    ...
    while (len > 0) {  // 循环读取，直到读取到足够的数据
        ...
        if (is->audio_buf_index >= is->audio_buf_size) {
            ...
            if (audio_size < 0) {
                ...
                is->audio_buf = NULL;
                is->audio_buf_size =
                    AUDIO_MIN_BUFFER_SIZE / is->audio_tgt.frame_size * is->audio_tgt.frame_size;
            ...
            if (is->ffp_get_playback_rate_change()) {
                ...
                // 再创建
                is->audio_speed_convert =
                    sonicCreateStream(is->get_target_frequency(), is->get_target_channels());
                // 2.设置倍速 音调 语速系数
            ...
            if (!is->is_normal_playback_rate() && is->audio_buf) {
                ...
                // 2通道  目前只支持2通道的
                out_size = (num_samples)*av_get_bytes_per_sample(is->audio_tgt.fmt) *
                           is->audio_tgt.channels;
                av_fast_malloc(&is->audio_buf1, &is->audio_buf1_size, out_size);
                if (out_ret) {
                    // 4.从流中读取处理好的数据
                    if (is->audio_tgt.fmt == AV_SAMPLE_FMT_FLT) {
                        sonic_samples = sonicReadFloatFromStream(
                            is->audio_speed_convert, (float*)is->audio_buf1, num_samples);
                    } else if (is->audio_tgt.fmt == AV_SAMPLE_FMT_S16) {
                        sonic_samples = sonicReadShortFromStream(
                            is->audio_speed_convert, (short*)is->audio_buf1, num_samples);
                    } else {
        ...
        if (!is->audio_muted_ && is->audio_buf && is->audio_volume == AUDIO_MIX_MAXVOLUME) {
            memcpy(stream, (uint8_t*)is->audio_buf + is->audio_buf_index, len1);
        } else {
            memset(stream, 0, len1);
            if (!is->audio_muted_ && is->audio_buf) {
                audio_mix_pcm(stream, (const uint8_t*)is->audio_buf + is->audio_buf_index, len1,
                              is->audio_volume);
            }
        }
    ...
    if (!std::isnan(is->audio_clock)) {
        ...
        set_clock_at(&is->audclk,
                     audio_clock - (double)(2 * is->audio_hw_buf_size + is->audio_write_buf_size) /
...
int FFPlayer::audio_open(const AVChannelLayout* wanted_channel_layout, int wanted_nb_channels,
                         int wanted_sample_rate, AudioParams* audio_hw_params) {
    ez_audio_params_uninit(audio_hw_params);
    audio_hw_params->fmt      = AV_SAMPLE_FMT_S16;
    audio_hw_params->freq     = wanted_sample_rate > 0 ? wanted_sample_rate : 44100;
    audio_hw_params->channels = wanted_nb_channels > 0 ? wanted_nb_channels : 2;
    if (audio_hw_params->channels > 2) {
        audio_hw_params->channels = 2;
    }

    int layout_ret = 0;
    if (wanted_channel_layout && wanted_channel_layout->nb_channels == audio_hw_params->channels) {
        layout_ret = av_channel_layout_copy(&audio_hw_params->ch_layout, wanted_channel_layout);
    } else {
        av_channel_layout_default(&audio_hw_params->ch_layout, audio_hw_params->channels);
    }
    if (layout_ret < 0) {
    ...
    // 默认以 2048 个采样点为一个处理周期
    int wanted_samples = 2048;
    int buffer_size =
        wanted_samples * audio_hw_params->channels * av_get_bytes_per_sample(audio_hw_params->fmt);
    audio_hw_buf_size = buffer_size;

#if defined(HAVE_WEBRTC_AV_CAPTURE)
    if (!audio_engine_) {
        webrtc_av::ApmConfig apm_cfg;
        audio_engine_ = webrtc_av::CreateAudioEngine(apm_cfg, nullptr);
    }
    if (audio_engine_) {
        audio_engine_->StartPlayout();
    }
#endif

    // 启动音频独立播放驱动线程
    if (audio_play_thread_ && audio_play_thread_->joinable()) {
        audio_play_running_ = false;
        frame_queue_signal(&sampq);
        audio_play_thread_->join();
    }
    audio_play_running_ = true;
    audio_play_paused_  = true;  // 等待外部 audio_pause(0) 唤醒播放
    audio_play_thread_  = std::make_unique<std::thread>(&FFPlayer::audio_play_loop, this);
...
#if defined(HAVE_WEBRTC_AV_CAPTURE)
    if (audio_engine_) {
        audio_engine_->StopPlayout();
    }
#endif
}

void FFPlayer::audio_play_loop() {
    int buf_size = audio_hw_buf_size > 0 ? audio_hw_buf_size : 8192;
    std::vector<uint8_t> pcm_buf(buf_size, 0);

    while (audio_play_running_) {
        if (audio_play_paused_ || paused) {
        ...
        if (buf_size != audio_hw_buf_size && audio_hw_buf_size > 0) {
            buf_size = audio_hw_buf_size;
            pcm_buf.resize(buf_size);
        }

        sdl_audio_callback(this, pcm_buf.data(), buf_size);

#if defined(HAVE_WEBRTC_AV_CAPTURE)
        if (audio_engine_) {
            int channels         = audio_tgt.channels > 0 ? audio_tgt.channels : 2;
            int sample_rate      = audio_tgt.freq > 0 ? audio_tgt.freq : 44100;
            int bytes_per_sample = av_get_bytes_per_sample(audio_tgt.fmt);
            if (bytes_per_sample <= 0) {
                bytes_per_sample = sizeof(int16_t);
            }
            size_t samples_per_channel = buf_size / (channels * bytes_per_sample);
            audio_engine_->PlayAudioData(reinterpret_cast<const int16_t*>(pcm_buf.data()),
                                         samples_per_channel, sample_rate, channels);
        }
#endif

        double duration_sec = 0.04;
        if (audio_tgt.bytes_per_sec > 0) {
            duration_sec = static_cast<double>(buf_size) / audio_tgt.bytes_per_sec;
        }
        auto target_time =
            start_time + std::chrono::microseconds(static_cast<int64_t>(duration_sec * 1000000.0));
...
void FFPlayer::audio_pause(int pause_on) {
    audio_play_paused_ = (pause_on != 0);
}
...
void FFPlayer::screenshot(AVFrame* frame) {
    if (req_screenshot_) {
        ...
        // 如果正常则ret = 0; 异常则为 < 0
        msg_queue_.get()->notify_msg(FFP_MSG_SCREENSHOT_COMPLETE, ret, 0, screen_path_,
                                     strlen(screen_path_) + 1);
        // 截屏完毕后允许再次截屏
...
void FFPlayer::sync_clock_to_slave(Clock* c, Clock* slave) {
    ...
    if (!isnan(slave_clock) &&
        (isnan(clock) || fabs(clock - slave_clock) > AV_NOSYNC_THRESHOLD))  //范围超过1.0 重新设置
    {
...
void FFPlayer::ffp_set_playback_volume(int value) {
    ...
    value        = av_clip(AUDIO_MIX_MAXVOLUME * value / 100, 0, AUDIO_MIX_MAXVOLUME);
    audio_volume = value;
...
void FFPlayer::ffp_frameq_cache(int value) {
    ...
    if (frameq_cache_flag &&
        (audioq.duration_cache_max + videoq.duration_cache_shake < stat.audio_cache.duration &&
    ...
    } else if (!frameq_cache_flag && (audioq.duration_cache_max - videoq.duration_cache_shake >
                                          stat.audio_cache.duration &&
                                      videoq.duration_cache_max - videoq.duration_cache_shake >
                                          stat.video_cache.duration)) {  //低于max - shake
        //投递恢复
        msg_queue_->notify_msg(FFP_MSG_FRAMEQ_CACHE_REGAIN);
        frameq_cache_flag = 1;
    }

    LOG(INFO) << "AUDIOQ dura" << stat.audio_cache.duration << "max a" << audioq.duration_cache_max
              << "shake a" << audioq.duration_cache_shake << "video dura"
              << stat.video_cache.duration << "max a" << videoq.duration_cache_max << "shakea"
              << videoq.duration_cache_shake << "flag" << frameq_cache_flag;
}
...
static int is_realtime(AVFormatContext* s) {
    if (!strcmp(s->iformat->name, "rtp") || !strcmp(s->iformat->name, "rtsp") ||
        !strcmp(s->iformat->name, "sdp") || !strcmp(s->iformat->name, "rtmp")) {
        return 1;
    }
...
int FFPlayer::read_thread() {
    ...
    while (1) {
        ...
        if (infinite_buffer < 1 && (audioq.size + videoq.size > MAX_QUEUE_SIZE ||
        ...
        if (ret < 0) {  // 出错或者已经读取完毕了
            ...
            if (ic->pb && ic->pb->error) {  // io异常 // 退出循环
                ...
                msg_queue_->notify_msg(FFP_MSG_NETWORK_URL_ERROR, 0, 0, error.data(),
                                       error.length());
                break;
            }
            std::this_thread::sleep_for(std::chrono::milliseconds(
                10));  // 读取完数据了，这里可以使用timeout的方式休眠等待下一步的检测
            continue;  // 继续循环
        } else {
            eof = 0;
        }
...
int FFPlayer::video_refresh_thread() {
    double remaining_time =
        0.0;  //vieopts-system  是个正数则表示超过了系统时钟，需要放慢 。是个负数则看情况进行drop。
    while (!abort_request) {
...
void FFPlayer::video_refresh(double* remaining_time) {
    ...
    retry:
        if (frame_queue_nb_remaining(&pictq) == 0) {
            ...
            if (vp->serial !=
                videoq
                    .serial) {  // 如果不是最新的播放序列，则将其从队列，以尽快读取最新的播放序列的Frame
            ...
            ez_mutex_lock(pictq.mutex);
            if (!std::isnan(vp->pts)) {
                update_video_pts(vp->pts, vp->pos, vp->serial);
            }
            ez_mutex_unlock(pictq.mutex);
...
int FFPlayer::initHWDecoder(AVCodecContext* avctx, const AVCodec* codec) {
    ...
    enum AVHWDeviceType type =
        av_hwdevice_find_type_by_name(hw_device_type.data());  // 根据名称自动寻找硬解码
    ...
    for (int i = 0;; i++) {
        const AVCodecHWConfig* config =
            avcodec_get_hw_config(codec, i);  // 检索编解码器支持的硬件配置。
        if (!config) {
            LOG(ERROR) << "打开硬件解码器失败！";
            m_isHw_device = false;
            ...
            config->device_type == type)  // 判断是否是设备类型
        {
            ...
            int ret =
                av_hwdevice_ctx_create(&hw_device_ctx, config->device_type, nullptr, nullptr, 0);
            if (ret < 0) {
...
int Decoder::decoder_start(AVMediaType codec_type, const char* thread_name, void* arg) {
    ...
    if (AVMEDIA_TYPE_VIDEO == codec_type) {
        decoder_thread_ = std::make_unique<std::thread>(&Decoder::video_thread, this, arg);
    } else if (AVMEDIA_TYPE_AUDIO == codec_type) {
        decoder_thread_ = std::make_unique<std::thread>(&Decoder::audio_thread, this, arg);
    } else {
        return -1;
    }
    return 0;
    ...
    for (;;) {
        ...
        do {
            ...
            if (queue_->serial != pkt_serial_) {
                // darren自己的代码
                LOG(INFO) << "discontinue:queue->serial:" << queue_->serial
                          << ", pkt_serial:" << pkt_serial_;
...
int Decoder::get_video_frame(AVFrame* frame) {
    ...
    */
    //2. frame drop 分析获取到的该帧是否要drop掉, 该机制的目的是在放入帧队列前先drop掉过时的视频帧
    if (got_picture) {
...
int Decoder::queue_picture(FrameQueue* fq, AVFrame* src_frame, double pts, double duration,
...
int Decoder::video_thread(void* arg) {
    ...
    AVFrame* tmp_frame = nullptr;  //硬件解码
    ...
    for (;;) {                        // 循环取出视频解码的帧数据
        ...
        //硬解2 从硬件buffer拷贝的内存来
        //这里可以考虑直接使用frame给显存暂时， 这里如果开启耗时增加%75左右
        if (is->m_isHw_device) {
        ...
        // 5 将解码后的视频帧插入队列
        ret = queue_picture(&is->pictq, tmp_frame, pts, duration, tmp_frame->pkt_pos,
                            is->viddec.pkt_serial_);
        // 6 释放frame对应的数据
