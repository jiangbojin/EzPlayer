...
ImageScaler::ImageScaler() {
    ...
    en_alogrithm_ = SwsAlogrithm::SWS_SA_FAST_BILINEAR;
...
RET_CODE ImageScaler::Init(uint32_t src_width, uint32_t src_height, int src_pix_fmt,
    ...
    sws_ctx_      = sws_getContext(src_width_, src_height_, (AVPixelFormat)src_pix_fmt_, dst_width_,
                                   dst_height_, (AVPixelFormat)dst_pix_fmt_, SWS_FAST_BILINEAR, NULL,
                                   NULL, NULL);
    if (!sws_ctx_) {
        LOG(ERROR) << "Impossible to create scale context for the conversion fmt:"
                   << av_get_pix_fmt_name((enum AVPixelFormat)src_pix_fmt_) << ", s:" << src_width_
                   << "x" << src_height_ << " -> fmt:" << av_get_pix_fmt_name(dst_pix_fmt_)
...
RET_CODE ImageScaler::Scale(const AVFrame* src_frame, AVFrame* dst_frame) {
    if (src_frame->width != src_width_ || src_frame->height != src_height_ ||
        src_frame->format != src_pix_fmt_ || dst_frame->width != dst_width_ ||
        dst_frame->height != dst_height_ || dst_frame->format != dst_pix_fmt_ || !sws_ctx_) {
        // 重新初始化
        DeInit();
        RET_CODE ret = Init(src_frame->width, src_frame->height, src_frame->format,
                            dst_frame->width, dst_frame->height, dst_frame->format, en_alogrithm_);
        if (ret != RET_CODE::RET_OK) {
            LOG(ERROR) << "Init failed: " << static_cast<int>(ret);
            return ret;
        }
    }
...
RET_CODE ImageScaler::Scale2(const VideoFrame* src_frame, VideoFrame* dst_frame) {
    ...
    int dst_slice_h = sws_scale(sws_ctx_, (const uint8_t**)src_frame->data, src_frame->linesize,
                                0,                  // 起始位置
...
RET_CODE ImageScaler::Scale3(const Frame* src_frame, VideoFrame* dst_frame) {
    ...
    int dst_slice_h =
        sws_scale(sws_ctx_, (const uint8_t**)src_frame->frame->data, src_frame->frame->linesize,
                  0,                  // 起始位置
                  src_frame->height,  //处理多少行
                  dst_frame->data, dst_frame->linesize);
    //LOG(INFO)<<"end";
