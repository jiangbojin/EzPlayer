...
class Decoder {
   public:
    ...
    int queue_picture(FrameQueue* fq, AVFrame* src_frame, double pts, double duration, int64_t pos,
                      int serial);
...
class FFPlayer {
   public:
    ...
    void RestTimeout() { pre_timeout_ = TimesUtil::GetTimeMillisecond(); }
    int64_t Get_BlockTimeout() { return TimesUtil::GetTimeMillisecond() - pre_timeout_; }
    ...
    int audio_open(const AVChannelLayout* wanted_channel_layout, int wanted_nb_channels,
                   int wanted_sample_rate, struct AudioParams* audio_hw_params);
    void audio_close();
    void audio_play_loop();
    void audio_pause(int pause_on);
    ...
    std::unique_ptr<std::thread> audio_play_thread_{nullptr};
    std::atomic<bool> audio_play_running_{false};
    std::atomic<bool> audio_play_paused_{true};
#if defined(HAVE_WEBRTC_AV_CAPTURE)
    webrtc_av::AudioEnginePtr audio_engine_{nullptr};
#endif
    ...
    struct AudioParams audio_tgt {};    // 保存音频输出需要的参数
    struct SwrContext* swr_ctx = NULL;  // 音频重采样context
    int audio_hw_buf_size      = 0;     // 音频缓冲区的大小(字节为单位)
    // 指向待播放的一帧音频数据，指向的数据区将被拷入音频缓冲区。若经过重采样则指向audio_buf1，
    // 否则指向frame中的音频
    ...
    int audio_buf_index = 0;  // 更新拷贝位置 当前音频帧中已拷入音频缓冲区
