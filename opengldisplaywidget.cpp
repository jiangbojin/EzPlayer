...
int OpenGLDisplayWidget::Draw(const Frame *frame) {
  ...
  bool need_realloc =
      (w != frame_width_ || h != frame_height_ || fmt != pixel_format_);
  ...
  if (fmt == AV_PIX_FMT_YUV420P) {
    ...
    if (avframe->linesize[1] == half_w) {
      ...
      for (int i = 0; i < half_h; i++) {
        memcpy(u_data_ + i * half_w,
               avframe->data[1] + i * avframe->linesize[1], half_w);
      }
    }
    u_linesize_ = half_w;
    ...
    if (avframe->linesize[2] == half_w) {
      ...
      for (int i = 0; i < half_h; i++) {
        memcpy(v_data_ + i * half_w,
               avframe->data[2] + i * avframe->linesize[2], half_w);
      }
    }
    v_linesize_ = half_w;
    ...
    if (avframe->linesize[1] == w) {
      ...
      for (int i = 0; i < half_h; i++) {
        memcpy(uv_data_ + i * w, avframe->data[1] + i * avframe->linesize[1],
               w);
      }
    }
    uv_linesize_ = w;
...
void OpenGLDisplayWidget::paintGL() {
  ...
  if (play_state_ != 1 || !has_new_frame_ || frame_width_ == 0 ||
      frame_height_ == 0) {
    return;
  }
  ...
  if (pixel_format_ == AV_PIX_FMT_YUV420P) {
    ...
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, frame_width_, frame_height_,
                    GL_LUMINANCE, GL_UNSIGNED_BYTE, y_data_);
    ...
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, frame_width_ / 2, frame_height_ / 2,
                    GL_LUMINANCE, GL_UNSIGNED_BYTE, u_data_);
    ...
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, frame_width_ / 2, frame_height_ / 2,
                    GL_LUMINANCE, GL_UNSIGNED_BYTE, v_data_);
  ...
  program->setAttributeBuffer("textureIn", GL_FLOAT, 2 * sizeof(GLfloat), 2,
                              4 * sizeof(GLfloat));
...
void OpenGLDisplayWidget::initShaders() {
  ...
  shader_program_yuv420p_->addShaderFromSourceCode(QOpenGLShader::Vertex,
                                                   vertexShaderSource);
  ...
  if (!shader_program_yuv420p_->link()) {
    qWarning() << "YUV420P Shader link failed:"
               << shader_program_yuv420p_->log();
  }

  // NV12 Shader
  shader_program_nv12_ = new QOpenGLShaderProgram(this);
  shader_program_nv12_->addShaderFromSourceCode(QOpenGLShader::Vertex,
                                                vertexShaderSource);
  shader_program_nv12_->addShaderFromSourceCode(QOpenGLShader::Fragment,
                                                fragmentShaderNV12);
...
void OpenGLDisplayWidget::initTextures() {
  ...
  if (pixel_format_ == AV_PIX_FMT_YUV420P) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, frame_width_, frame_height_, 0,
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, frame_width_ / 2,
                 frame_height_ / 2, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, nullptr);
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE_ALPHA, frame_width_ / 2,
                 frame_height_ / 2, 0, GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE,
                 nullptr);
  }
}
